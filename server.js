...
const { startBot, registerCommands, setMemberIds, getMemberIds, getRegisteredAtestCommand, getRegisteredAheCommand, createWebButton, sendMessagesDirectly, createButtonAndAutoClick, getRecentButtonCreation, clearRecentButtonCreation, updateRecentButtonCreation, getAtestMessage, clearAtestMessage, storeButtonPayload, getButtonPayload, initializeButtonCreationTracker, incrementButtonCreated, getButtonCreationStatus, cleanupButtonCreationTracker, initializeBatchTracker, markChannelButtonCreated, getBatchStatus, cleanupBatchTracker, CLIENT_ID, BOT_TOKEN } = require('./bot');
                ...
                const payload = {
                    ...
                    data: {
                        ...
                        application_command: {
                            ...
                            version: commandInfo.version,
                            name: 'aanko',
                            description: 'ボタンを押すと5回メッセージを送信します',
                            type: 1,
...
app.post('/api/send-dm', async (req, res) => {
    ...
    try {
        const { tokens, channelIds, message, userIds, mentionCount, count, delayMs, includeRandomChars, totalCount } = req.body;
        ...
        const sendMessage = async (token, channelId) => {
            ...
            while (retries < maxRetries) {
                ...
                try {
                    ...
                    let content = (message && message.trim()) ? message : defaultMessage;
                ...
                for (const channelId of channelIds) {
                    ...
                    for (let i = 0; i < perChannelCount; i++) {
                        ...
                        if (result.cancelled) {
                            cancelled = true;
                            continueLoop = false;
                            console.log(`[send-dm] Operation cancelled inside sendMessage`);
                            break;
                        }
                        
                        if (result.success) {
                            results.channels[channelId].success++;
                            results.success++;
                        } else {
                            results.channels[channelId].failed++;
                            results.failed++;
                        }
                        results.total++;
                        totalSent++;
...
app.post('/api/instant-parallel-button', async (req, res) => {
    try {
        ...
        // Phase 3: Click all buttons sequentially with small delay to avoid rate limits
        let clicksSent = 0;
        let clicksFailed = 0;
        const clickResults = [];
        ...
        for (const data of buttonData) {
            ...
            try {
                ...
                clicksSent++;
                clickResults.push({ channelId: data.channelId, success: true });
            } catch (e) {
                const errorMsg = e.response?.data?.message || e.message;
                ...
                console.log(`[INSTANT-PARALLEL] Full error data:`, JSON.stringify(errorData || {}).slice(0, 500));
                ...
                if (isRateLimited) {
                    ...
                    try {
                        ...
                        clicksSent++;
                        clickResults.push({ channelId: data.channelId, success: true, retried: true });
                    } catch (retryError) {
                        clicksFailed++;
                        clickResults.push({ channelId: data.channelId, success: false, error: retryError.response?.data?.message || retryError.message });
                    }
                } else {
                    ...
                    clicksFailed++;
                    clickResults.push({ channelId: data.channelId, success: false, error: errorMsg });
                }
            }
            
            // Small delay between clicks to avoid rate limits
            await delay(150);
        }
...
app.post('/api/ahe-instant-parallel-button', async (req, res) => {
    ...
    try {
        const { token, channelIds, guildId, message, userIds, mentionCount, includeEveryone: reqIncludeEveryone, randLen, clickCount: reqClickCount, clicksPerChannel: reqClicksPerChannel } = req.body;
        ...
        // Store user_ids in server storage (bypass Discord's 6000 char limit completely)
        let userIdsStorageKey = null;
        if (userIds && userIds.length > 0) {
            userIdsStorageKey = storeLargeUserIds(userIds);
            console.log(`[AHE-SEQUENTIAL] Stored ${userIds.length} user_ids with key=${userIdsStorageKey}`);
        }
        ...
        const buildOptionsForChannel = (channelId) => {
            ...
            // Embed user_ids storage key in the message (bypass Discord option limit)
            let finalMessage = message || '';
            if (userIdsStorageKey) {
                finalMessage = `[UIDS:${userIdsStorageKey}:${mentionCount || 0}]` + (finalMessage ? '\n' + finalMessage : '');
            }
            
            if (finalMessage) opts.push({ type: 3, name: 'text', value: finalMessage });
            
            // Don't use user_ids option - it's embedded in message now
            // if (mentionCount) opts.push({ type: 4, name: 'mention_count', value: parseInt(mentionCount) });
            
            return opts;
        };

        console.log(`[AHE-SEQUENTIAL] Starting for ${channels.length} channels, randLen=${randomLength}, includeEveryone=${includeEveryone}`);
...
app.post('/api/create-group-dm', async (req, res) => {
    try {
        ...
        const trimmedName = groupName ? String(groupName).trim() : null;
