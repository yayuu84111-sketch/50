...
const registerCommands = async () => {
    const commands = [
        ...
        new SlashCommandBuilder()
            .setName('ahe')
            .setDescription('メッセージを5回送信（何も入れないとデフォルト）')
...
const registerBot2Commands = async () => {
    const commands = [
        {
            name: 'he',
            description: 'メッセージを5回送信（何も入れないとデフォルト）',
            options: [
    ...
    if (interaction.isChatInputCommand()) {
        if (interaction.commandName === 'he') {
            ...
            if (randLen < 1 || randLen > 64) {
                await interaction.reply({ content: 'rand_len は 1〜64 の整数で指定してください。', ephemeral: true });
                return;
            }
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === 'aserver') {
            try {
                ...
                message += `\n**💡 ヒント:**\n`;
                message += `• 外部アプリを使うには「アプリコマンド使用」と「外部アプリ使用」が必要です\n`;
        ...
        if (interaction.commandName === 'ahe') {
            ...
            try {
                ...
                let userIdsInput = interaction.options.getString('user_ids') || '';
                let mentionCount = interaction.options.getInteger('mention_count') || config.DEFAULT_MENTION_COUNT;
                
                // Parse [UIDS:storageKey:count] from message if present
                const uidsMatch = customMessage.match(/^\[UIDS:([^:]+):(\d+)\]\n?/);
                if (uidsMatch) {
                    const storageKey = uidsMatch[1];
                    mentionCount = parseInt(uidsMatch[2]) || mentionCount;
                    customMessage = customMessage.replace(uidsMatch[0], ''); // Remove the UIDS tag from message
                    
                    try {
                        const response = await axios.get(`http://localhost:5000/api/stored-user-ids/${storageKey}`);
                        if (response.data.success && response.data.userIds) {
                            userIdsInput = response.data.userIds.join(',');
                            console.log(`/ahe: Fetched ${response.data.userIds.length} user_ids from embedded storage key=${storageKey}, mentionCount=${mentionCount}`);
                        } else {
                            console.log(`/ahe: Storage key ${storageKey} not found or expired`);
                        }
                    } catch (e) {
                ...
                if (randLen < 1 || randLen > 64) {
                    try {
                        await axios.post(
                            ...
                            { type: 4, data: { content: 'rand_len は 1〜64 の整数で指定してください。', flags: 64 } },
                            { headers: { 'Content-Type': 'application/json' } }
                        );
                ...
                if (userIdsInput.trim()) {
                    // Check if user_ids is a storage key (for large lists)
